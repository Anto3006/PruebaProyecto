...
namespace Planetario.Handlers {
  public class ActividadHandler : BaseDeDatosHandler{
    ...
    public bool CrearActividad(ActividadModel actividad) {
      ...
      String consulta = "INSERT INTO Actividad " + "(tituloPK,fechaPK,descripcion,precioSugerido,duracion,estado,correo,nivelDeComplejidad,tipoDeActividad,modalidad,numeroDeIdentificacionFuncionarioFK, enlaceStream)" +
         " VALUES (@titulo,@fecha,@descripcion,@precioSugerido,@duracion, " +
                         "@estado,@correo,@nivelDeComplejidad,@tipoActividad,@modalidad,@numeroDeIdentificacionFuncionarioFK, @enlaceStream)";
      bool exitoAgregarActividad = AgregarActividad(consulta, actividad);
      if (!exitoAgregarActividad) {
        return exitoAgregarActividad;
      }
      bool exitoAgregarPublicoMeta = AgregarPublicoMeta(actividad);
      if (!exitoAgregarPublicoMeta) {
        return exitoAgregarPublicoMeta;
      }
      bool exitoAgregarTopicos = AgregarTopicosDeActividad(actividad);
      return exitoAgregarTopicos;
    }
